...
impl TokenWhitelistContract {
    ...
    pub fn remove_token(env: Env, admin: Address, token: Address) {
        ...
        if env
            .storage()
            .persistent()
            .has(&DataKey::SuspensionRecord(token.clone()))
        {
            env.storage()
                .persistent()
    ...
    pub fn set_risk_tier(
        ...
        env.storage()
            .instance()
            .set(&DataKey::RiskTier(tier_id), &limits);
        env.storage()
            .instance()
            .extend_ttl(INSTANCE_LIFETIME_THRESHOLD, INSTANCE_BUMP_AMOUNT);
    ...
    pub fn assign_token_tier(env: Env, admin: Address, token: Address, tier_id: u32) {
        ...
        if env
            .storage()
            .instance()
            .get::<_, TierLimits>(&DataKey::RiskTier(tier_id))
            .is_none()
        {
        ...
        env.storage()
            .persistent()
            .set(&DataKey::TokenTier(token.clone()), &tier_id);
        env.storage().persistent().extend_ttl(
            &DataKey::TokenTier(token.clone()),
            PERSISTENT_LIFETIME_THRESHOLD,
            PERSISTENT_BUMP_AMOUNT,
        );
    ...
    pub fn set_token_limit_override(
        env: Env,
        admin: Address,
        token: Address,
        max_single_tx_amount: i128,
        ...
        Self::require_admin(&env, &admin);
        let limits = TierLimits {
            name: soroban_sdk::String::from_str(&env, "override"),
            max_single_tx_amount,
            max_daily_volume,
        };
        env.storage()
            .persistent()
            .set(&DataKey::TokenLimitOverride(token.clone()), &limits);
        env.storage().persistent().extend_ttl(
            &DataKey::TokenLimitOverride(token.clone()),
            PERSISTENT_LIFETIME_THRESHOLD,
            PERSISTENT_BUMP_AMOUNT,
        );
    ...
    pub fn get_token_tier_limits(env: Env, token: Address) -> TierLimits {
        if let Some(override_limits) = env
            .storage()
            .persistent()
            .get::<_, TierLimits>(&DataKey::TokenLimitOverride(token.clone()))
        {
            return override_limits;
        }
        if let Some(tier_id) = env
            .storage()
            .persistent()
            .get::<_, u32>(&DataKey::TokenTier(token.clone()))
        {
        ...
        if let Some(limits) = env
            .storage()
            .instance()
            .get::<_, TierLimits>(&DataKey::RiskTier(1u32))
        {
            return limits;
        }
        TierLimits {
            name: soroban_sdk::String::from_str(&env, "tier-default"),
    ...
    pub fn get_token_limit_override(env: Env, token: Address) -> Option<TierLimits> {
        env.storage()
            .persistent()
            .get(&DataKey::TokenLimitOverride(token))
    }
    ...
    pub fn set_token_metadata(
        ...
        logo_hash: BytesN<32>,
        canonical_oracle: Option<Address>,
    ) {
        admin.require_auth();
        Self::require_admin(&env, &admin);
        ...
        env.storage()
            .persistent()
            .set(&DataKey::TokenMetadata(token.clone()), &md);
        env.storage().persistent().extend_ttl(
            &DataKey::TokenMetadata(token.clone()),
            PERSISTENT_LIFETIME_THRESHOLD,
            PERSISTENT_BUMP_AMOUNT,
        );
    ...
    pub fn get_token_metadata(env: Env, token: Address) -> TokenMetadata {
        if !env
            .storage()
            .persistent()
            .has(&DataKey::WhitelistMembership(token.clone()))
        {
    ...
    pub fn update_token_decimals(env: Env, admin: Address, token: Address, decimals: u32) {
        admin.require_auth();
        Self::require_admin(&env, &admin);
        let mut md: TokenMetadata = env
            .storage()
            .persistent()
            .get(&DataKey::TokenMetadata(token.clone()))
            .expect("Metadata not set");
    ...
    pub fn get_all_token_metadata(env: Env, offset: u32, limit: u32) -> Vec<TokenMetadata> {
        ...
        for i in start..start + l {
            ...
            if let Some(md) = env
                .storage()
                .persistent()
                .get::<_, TokenMetadata>(&DataKey::TokenMetadata(t.clone()))
            {
                res.push_back(md);
            }
        }
    ...
    pub fn is_whitelisted(env: Env, token: Address) -> bool {
        env.storage()
            .persistent()
            .has(&DataKey::WhitelistMembership(token))
    }
    ...
    pub fn set_contract_token(
        ...
        env.storage()
            .instance()
            .extend_ttl(INSTANCE_LIFETIME_THRESHOLD, INSTANCE_BUMP_AMOUNT);
        events::emit_contract_token_allowlist_updated(
            &env,
            contract_id,
            token,
            true,
    ...
    pub fn cleanup_allowlist_entries(env: Env, entries: Vec<(Address, Address)>) {
        let current_ledger = env.ledger().sequence();
    ...
    pub fn set_token_quota(
        ...
        if period_ledgers > MAX_QUOTA_PERIOD_LEDGERS {
            panic!("period_ledgers exceeds maximum allowed");
        }
    ...
    pub fn record_token_volume(env: Env, token: Address, amount: i128) -> Result<(), Error> {
        ...
        let Some(quota) = env
            .storage()
            .persistent()
            .get::<_, TokenQuota>(&DataKey::TokenQuota(token.clone()))
        else {
            return Ok(());
        };
        let current_ledger = env.ledger().sequence();
    ...
    pub fn set_min_proposal_stake(env: Env, admin: Address, min_stake: i128) {
        ...
        Self::require_admin(&env, &admin);
        if min_stake <= 0 {
            panic!("min_stake must be positive");
        }
        env.storage()
            .instance()
            .set(&DataKey::MinProposalStake, &min_stake);
        env.storage()
            .instance()
            .extend_ttl(INSTANCE_LIFETIME_THRESHOLD, INSTANCE_BUMP_AMOUNT);
    }
    ...
    pub fn set_voting_window_ledgers(env: Env, admin: Address, ledgers: u32) {
        ...
        env.storage()
            .instance()
            .set(&DataKey::VotingWindowLedgers, &ledgers);
        env.storage()
            .instance()
            .extend_ttl(INSTANCE_LIFETIME_THRESHOLD, INSTANCE_BUMP_AMOUNT);
    }
    ...
    pub fn set_enactment_delay_ledgers(env: Env, admin: Address, ledgers: u32) {
        ...
        env.storage()
            .instance()
            .set(&DataKey::EnactmentDelayLedgers, &ledgers);
        env.storage()
            .instance()
            .extend_ttl(INSTANCE_LIFETIME_THRESHOLD, INSTANCE_BUMP_AMOUNT);
    }
    ...
    pub fn set_quorum_bps(env: Env, admin: Address, quorum_bps: u32) {
        ...
        if quorum_bps > 10_000 {
            panic!("quorum_bps cannot exceed 10000");
        }
        env.storage()
            .instance()
    ...
    pub fn get_governance_config(env: Env) -> (Option<Address>, i128, u32, u32, u32) {
        let governance_token: Option<Address> =
    ...
    pub fn propose_token_listing(
        ...
        env.storage()
            .persistent()
            .set(&DataKey::ListingProposal(proposal_id), &proposal);
        env.storage().persistent().extend_ttl(
    ...
    pub fn get_vote_record(env: Env, proposal_id: u32, voter: Address) -> Option<bool> {
        env.storage()
            .persistent()
            .get::<DataKey, (bool, i128)>(&DataKey::VoteRecord(proposal_id, voter))
            .map(|(approve, _)| approve)
    }
    ...
    pub fn get_vote_weight_snapshot(env: Env, proposal_id: u32, voter: Address) -> Option<i128> {
        env.storage()
            .persistent()
            .get(&DataKey::VoteWeightSnapshot(proposal_id, voter))
    }
}
